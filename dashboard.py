...
GOOGLE_SHEET_ID = "19c9CcFXaSLsj-E0y2qw0LQNUxh9X-xTR"
    ...
    with col:
        st.markdown(
            ...
            <div class="kpi-card">
                ...
                <div class="kpi-sub">Acum. Mes: {acum if acum else '-'}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def render_table_panel(title, df):
    ...
    for c, h in zip(header_cols, ["Indicador", "Hoy", "Meta", "", "Tendencia"]):
    ...
    for _, r in df.iterrows():
        ...
        vals = synth_trend(r["indicador"], r["hoy"])
        if vals:
...
def render_area_card(title, stats, df, extra_html=""):
    ...
    if stats:
        ...
        for c, (label, val) in zip(cols, stats):
            disp = f"{val*100:.0f}%" if isinstance(val, float) and val < 1.5 and "%" not in str(label) and label.lower().startswith(("%",)) else val
            if isinstance(val, float) and val < 1.5 and val != int(val):
                disp = f"{val*100:.1f}%"
...
def render_andenes_donut(andenes):
    ...
    values = [andenes.get(l, 0) for l in labels]
    util = andenes.get("% Utilización (calculado)", "-")
...
with h2:
    st.selectbox("CD", ["Todos"], label_visibility="collapsed")
with h3:
    st.selectbox("Periodo", ["Hoy"], label_visibility="collapsed")
with h4:
...
with p1:
    render_table_panel("SERVICIO AL CLIENTE", data["servicio"])
with p2:
    render_table_panel("PRODUCTIVIDAD OPERATIVA", data["productividad"])
with p3:
    render_table_panel("INVENTARIOS", data["inventarios"])
with p4:
    render_table_panel("COSTOS", data["costos"])
...
with a1:
    render_area_card("RECEPCIÓN", data["recepcion_stats"], data["recepcion_tabla"])
with a2:
    render_area_card("PICKING", data["picking_stats"], data["picking_tabla"])
with a3:
    st.markdown('<div class="panel"><div class="panel-title">ANDENES / MUELLES</div>', unsafe_allow_html=True)
    andenes = data["andenes"]
    c1, c2 = st.columns(2)
    c1.markdown(f"<div class='stat-box'><div class='stat-num'>{int(andenes.get('Total Andenes',0))}</div><div class='stat-label'>Total</div></div>", unsafe_allow_html=True)
    c2.markdown(f"<div class='stat-box'><div class='stat-num'>{int(andenes.get('En Uso',0))}</div><div class='stat-label'>En Uso</div></div>", unsafe_allow_html=True)
    st.plotly_chart(render_andenes_donut(andenes), use_container_width=True, config={"displayModeBar": False}, key="donut-andenes")
    st.markdown("</div>", unsafe_allow_html=True)
with a4:
    render_area_card("TRANSPORTE Y DISTRIBUCIÓN", data["transporte_stats"], data["transporte_tabla"])
with a5:
    render_area_card("E-COMMERCE", data["ecommerce_stats"], data["ecommerce_tabla"])
...
with b1:
    ...
    for num, texto in data["alertas"]:
        st.markdown(
            f"""<div class="alert-item"><span class="alert-num">{int(num)}</span>{texto}</div>""",
            unsafe_allow_html=True,
        )
    st.markdown("</div>", unsafe_allow_html=True)
...
with b2:
    ...
    st.caption("Tendencia ilustrativa: agrega una hoja 'Histórico' con valores diarios para datos reales.")
    fig = go.Figure()
    trend_series = [
        ("OTIF (%)", data["kpis"].loc[data["kpis"]["indicador"].str.contains("OTIF", case=False), "hoy"].values, COLOR_ACCENT),
        ("Costo Op. x Caja ($)", data["kpis"].loc[data["kpis"]["indicador"].str.contains("Costo Operativo", case=False), "hoy"].values, COLOR_RED),
    ]
    for name, arr, color in trend_series:
        if len(arr):
            ys = synth_trend(name, arr[0], n=14)
            fig.add_trace(go.Scatter(y=ys, mode="lines+markers", name=name, line=dict(color=color, width=2)))
    fig.update_layout(
        height=230,
...
with b3:
    ...
    for indicador, valor, estado in data["resumen"]:
        ...
        st.markdown(
            f"""<div class="row-line">
                <span>{indicador}</span>
                <span style="color:{color};font-weight:700;">{valor} {icon}</span>
            </div>""",
            unsafe_allow_html=True,
        )
    st.markdown("</div>", unsafe_allow_html=True)
