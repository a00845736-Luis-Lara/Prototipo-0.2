...
def render_table_panel(title, df, historico=None):
    ...
    for c, h in zip(header_cols, ["Indicador", "Hoy", "Meta", "", "Tendencia (7d)"]):
    ...
    for _, r in df.iterrows():
        ...
        vals = get_trend(historico, r["indicador"], r["hoy"], n=7)
        if vals:
...
with p1:
    render_table_panel("SERVICIO AL CLIENTE", data["servicio"], data["historico"])
with p2:
    render_table_panel("PRODUCTIVIDAD OPERATIVA", data["productividad"], data["historico"])
with p3:
    render_table_panel("INVENTARIOS", data["inventarios"], data["historico"])
with p4:
    render_table_panel("COSTOS", data["costos"], data["historico"])
...
with b2:
    ...
    st.caption("Últimos 30 días, tomados de la hoja 'Histórico' del Excel.")
    fig = go.Figure()
    trend_series = [
    ...
    for name, ind_arr, arr, color in trend_series:
        if len(arr):
            ys = get_trend(data["historico"], ind_arr[0], arr[0], n=30)
